...
from config import UPLOAD_FILE_TRANSLATE_DIR
from server.forms import FileUploadForm
from ai_asr.inference_tr import WhisperInferencetr
from server.views.sql_database import db, User, AudioFile

bp = Blueprint('File_Translation', __name__, url_prefix='/File_Translation')
...
def index():
    ...
    form = FileUploadForm()
    return render_template('File_Translation.html', form=form, username=username)
...
@bp.route('/upload/files/Translation', methods=['POST'])
def upload():
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'status': 'fail', 'contents': '로그인이 필요합니다.'}), 403

    user_upload_dir = create_time_based_directory(UPLOAD_FILE_TRANSLATE_DIR)
...
def process():
    ...
    # 결과 저장할 디렉토리 생성 (시간기반_디렉토리_Translation 형식)
    result_dir = user_audio_dir + "_Translation"
    ...
    for idx, audio in enumerate(audio_files):
        try:
            whisper = WhisperInferencetr()
            target_file = os.path.join(user_audio_dir, audio)
            transcript = whisper.inference_tr(target_file)
            transcript = transcript.replace('. ', '.\n')

            result_filepath = os.path.join(result_dir, f"{audio}_Translation.txt")
